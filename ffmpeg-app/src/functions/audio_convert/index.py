...
def handler(event, context):
    ...
    cmd = ['ffmpeg', '-i', input_path, '/tmp/{0}{1}'.format(shortname, dst_type)]
    if ac:
        if ar:
            cmd = ['ffmpeg', '-i', input_path, "-ac", str(ac), "-ar", str(ar),  '/tmp/{0}{1}'.format(shortname, dst_type)]
        else:
            cmd = ['ffmpeg', '-i', input_path, "-ac", str(ac), '/tmp/{0}{1}'.format(shortname, dst_type)]
    else:
        if ar:
            cmd = ['ffmpeg', '-i', input_path, "-ar", str(ar),  '/tmp/{0}{1}'.format(shortname, dst_type)]
